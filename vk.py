...
def find_max_size(dic_search):
    max_size = 0
    for i in range(len(dic_search)):
        file_size = dic_search[i].get('width') * dic_search[i].get('height')
        if file_size > max_size:
            max_size = file_size
            need_elem = i
    return dic_search[need_elem].get('url'), dic_search[need_elem].get('type')
        ...
        for elem in picture_dict.keys():
            for value in picture_dict[elem]:
                if len(picture_dict[elem]) == 1:
                    ...
                    f_name = f'{elem} {value["add_name"]}.jpeg'
                json_lst.append({'file name': f_name, 'size': value["size"]})
                sorted_dict[f_name] = picture_dict[elem][0]['url_picture']
